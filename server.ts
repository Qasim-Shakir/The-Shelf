...
// Ensure query indexes on commonly filtered/sorted fields
mongoose.connection.once("open", async () => {
  try {
    ...
    await Book.collection.createIndex({ ingestedAt: -1 });
    console.log("[Index] Book query indexes ensured");
  } catch (err: any) {
    console.warn("[Index] Could not create indexes:", err.message);
  }
});
  ...
  app.post("/api/auth/signup", async (req, res) => {
    try {
      ...
      const token = jwt.sign(
        ...
        JWT_SECRET,
        { expiresIn: SESSION_DURATION as jwt.SignOptions["expiresIn"] }
      );
      res.json({
        token,
  ...
  app.get("/api/books", async (req, res) => {
    try {
      ...
      const [books, total] = await Promise.all([
        Book.find().sort({ ingestedAt: -1 }).skip(skip).limit(limit).lean(),
        Book.countDocuments(),
      ]);
